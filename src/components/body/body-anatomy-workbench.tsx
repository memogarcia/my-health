...
import { bodyRegionAt } from "../../body-region";
import { t } from "../../i18n";
...
export function AnatomyStage({ controller }: { controller: DashboardController }) {
  ...
  const anatomyImageUrl = showsWomenBody ? anatomyWomenBodyUrl : anatomyBodyUrl;
  const visibleNotes = controller.userState.bodyNotes.filter((note) => Math.round(note.angle / 90) % 4 === 0);

  function createNote(event: React.MouseEvent<HTMLButtonElement>): void {
    const bounds = event.currentTarget.getBoundingClientRect();
    const x = Math.min(100, Math.max(0, ((event.clientX - bounds.left) / bounds.width) * 100));
    const y = Math.min(100, Math.max(0, ((event.clientY - bounds.top) / bounds.height) * 100));
    const area = bodyRegionAt(x, y, 0);
    controller.openBodyNote({
      area: t("body.notes.area", { area: t(`body.notes.region.${area.region}` as "body.notes.region.head"), view: t("body.notes.view.front") }),
      angle: 0,
      x,
      y,
    });
  ...
  return (
    <div className="anatomy-stage" aria-label={t("body.anatomy.label")}>
      <div className="anatomy-image-plane">
        <img alt={t(showsWomenBody ? "body.anatomy.altFemale" : "body.anatomy.alt")} className="anatomy-static-image" src={anatomyImageUrl} />
        <div className="anatomy-interaction-layer">
          <button aria-label={t("body.notes.createAt", { view: t("body.notes.view.front") })} className="anatomy-note-surface" onClick={createNote} type="button" />

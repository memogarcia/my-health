  ...
  return (
    <article className={cn("self-start flex gap-3.5 w-full", message.isError && "rounded-lg border border-attention/30 bg-attention/5 p-4")}>
      <div className={cn(
        ...
        message.isError ? "bg-attention/10 text-attention" : "bg-accent-soft text-accent"
      )}>
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      <div className="grid max-w-sm justify-items-center gap-3">
        <span className="grid size-12 place-items-center rounded-full bg-accent-soft text-accent shadow-sm border border-accent/15"><Sparkles className="size-5" /></span>
...
export function PendingMessage() {
  return (
    <article className="self-start flex gap-3.5 w-full animate-pulse">
      <div className="flex size-7 shrink-0 select-none items-center justify-center rounded-full bg-accent-soft/30 text-accent shadow-sm"><LoaderCircle className="size-3.5 animate-spin" /></div>
